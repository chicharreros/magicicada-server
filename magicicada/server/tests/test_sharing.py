...
class TestCreateShare(TestWithDatabase):
    ...
    def test_delete_share(self):
        ...
        def do_delete(client):
            ...
            res = yield client.create_share(
                root, self.usr1.username, "name", Share.VIEW
            )
            share_id = uuid.UUID(res.share_id)
            yield client.delete_share(share_id)
            self.assertRaises(
                errors.DoesNotExist, self.usr1.get_share, share_id
            )
    ...
    def test_delete_volume(self):
        ...
        def create_share(_):
            share = self.usr1.root.share(
                self.usr0.id, "sharename", readonly=True
            )
        ...
        def auth(client):
            ...
            d.addCallbacks(
                lambda r: client.create_share(
                    r, self.usr1.username, "name", Share.VIEW
                ),
                client.test_fail,
            )
    ...
    def test_create_basic_modify(self):
        ...
        def auth(client):
            ...
            d.addCallbacks(
                lambda r: client.create_share(
                    r, self.usr1.username, "name", Share.MODIFY
                ),
                client.test_fail,
            )
    ...
    def test_create_usr_twice(self):
        ...
        def auth(client):
            ...
            d.addCallbacks(
                lambda r: client.create_share(
                    r, self.usr1.username, "name1", Share.MODIFY
                ),
                client.test_fail,
            )
            ...
            d.addCallbacks(
                lambda r: client.create_share(
                    r, self.usr1.username, "name2", Share.MODIFY
                ),
                client.test_fail,
            )
    ...
    def test_create_different_name_same_user(self):
        ...
        def auth(client):
            ...
            d.addCallbacks(
                lambda r: client.create_share(
                    r.new_id, self.usr1.username, "new dir", Share.VIEW
                ),
                client.test_fail,
            )
    ...
    def test_create_same_name_same_user(self):
        ...
        def test(client):
            ...
            # create other directory to share
            makedir_req = yield client.make_dir(request.ROOT, root_id, "hola")

            # create the share using the just created directory
            yield client.create_share(
                makedir_req.new_id, self.usr1.username, "same_name", Share.VIEW
            )

            # create the share using root
            d = client.create_share(
                root_id, self.usr1.username, "same_name", Share.VIEW
            )
    ...
    def test_create_share_file(self):
        ...
        def auth(client):
            ...
            d.addCallbacks(
                lambda r: client.create_share(
                    r.new_id, self.usr1.username, "foo", Share.MODIFY
                ),
                client.test_fail,
            )
    ...
    def test_notify_creation_to_myself(self):
        ...
        def auth(client):
            def notif(share_notif):
                self.assertEqual(share_notif.subtree, str(self._state.root_id))
                self.assertEqual(share_notif.share_name, "name")
                self.assertEqual(share_notif.from_username, self.usr0.username)
                self.assertEqual(
                    share_notif.from_visible_name, self.usr0.visible_name
                )
            ...
            d.addCallbacks(
                lambda r: client.set_share_change_callback(notif),
                client.test_fail,
            )
            d.addCallbacks(
                lambda _: client.create_share(
                    self._state.root_id, "usr0", "name", Share.VIEW
                ),
                client.test_fail,
            )
    ...
    def test_notify_creation_to_myself_accepted_yes(self):
        ...
        def auth(client):
            def notif_share(share_notif):
                ...
                share = shares[0]
                self.assertEqual(share.name, "acc Y")
    ...
    def test_notify_creation_to_myself_accepted_no(self):
        ...
        def auth(client):
            ...
            def notif_answer(share_id, answer):
                ...
                # check the DB
                self.assertRaises(
                    errors.DoesNotExist, self.usr0.get_share, share_id
                )
    ...
    def test_notify_creation_bad(self):
        ...
        def auth(client):
            ...
            d.addCallbacks(
                lambda _: client.create_share(
                    self._state.root_id, self.usr1.username, "name", Share.VIEW
                ),
                client.test_fail,
            )
    ...
    def test_accept_dead_share(self):
        ...
        share = self.usr0.root.share(self.usr1.id, "a dead share")
        share.delete()
        ...
        def auth(client):
            ...
            d.addCallbacks(
                lambda _: client.accept_share(share.id, "Yes"),
                client.test_fail,
            )
...
class TestSharesWithData(TestWithDatabase):
    ...
    def setUp(self):
        ...
        filero = root2.make_file("file")
        rwdir = root2.make_subdirectory("rwdir")
        filerw = rwdir.make_file("file")
        subdir = root2.make_subdirectory("subdir")
        subfile = subdir.make_file("subfile")
        subsubdir = subdir.make_subdirectory("subsubdir")
        subsubfile = subsubdir.make_file("subsubfile")

        share = root2.share(self.usr0.id, "foo", readonly=True)
        share_other = root2.share(self.usr1.id, "foo", readonly=True)
        subshare = subdir.share(self.usr0.id, "foo2", readonly=True)
        share_owner = root1.share(self.usr0.id, "foo3", readonly=True)
        share_modify = rwdir.share(self.usr0.id, "foo4")
    ...
    def test_unlink_on_share_ro(self):
        ...
        def auth(client):
            """auth"""
            d = client.dummy_authenticate("open sesame")
            d.addCallback(lambda r: client.unlink(self.share, self.filero))
            d.addCallbacks(client.test_fail, lambda x: client.test_done())
    ...
    def test_move_on_share_from_mine(self):
        ...
        def auth(client):
            ...
            d.addCallback(lambda r: client.make_file("", r, "file"))
            d.addCallback(
                lambda r: client.move(
...
class TestListShares(TestWithDatabase):
    ...
    def test_one_share_view(self):
        ...
        def check(resp):
            ...
            self.assertEqual(share.other_username, "usr1")
    ...
    def test_one_share_modify(self):
        ...
        def auth(client):
            ...
            d.addCallbacks(
                lambda r: client.create_share(
                    r, self.usr1.username, "n1", Share.MODIFY
                ),
                client.test_fail,
            )
    ...
    def _create_share(self, _, accepted=False):
        ...
        share = root.share(self.usr0.id, "sharename", readonly=True)
        if accepted:
            self.usr0.get_share(share.id).accept()
    ...
    def test_several_shares(self):
        ...
        def createShare():
            self.usr1.root.share(self.usr0.id, "share 3", readonly=True)
    ...
    def test_share_offer(self):
        ...
        def createShareOffer():
            # test a share offer
            self.usr0.root.make_shareoffer(
                "me@example.com", "share Offer", readonly=True
            )
        ...
        def check(resp):
            ...
            self.assertEqual(share.direction, "from_me")
            self.assertEqual(share.other_username, "")
            self.assertEqual(share.access_level, Share.VIEW)
    ...
    def test_dead_share(self):
        ...
        def createShare():
            share = self.usr1.root.share(
                self.usr0.id, "another dead share", readonly=True
            )
            share.delete()
    ...
    def test_share_in_root_from_me_with_volume_id(self):
        ...
        def auth(client):
            ...
            root_id = yield client.get_root()
            yield client.create_share(
                root_id, self.usr1.username, "n1", Share.VIEW
            )
            # list the shares and check
    ...
    def test_share_in_udf_from_me_with_volume_id(self):
        ...
        def auth(client):
            ...
            udf = yield client.create_udf("~/myudf", "foo")
            yield client.create_share(
                udf.node_id, self.usr1.username, "n1", Share.VIEW
            )
            # list the shares and check
            resp = yield client.list_shares()
            self.assertEqual(len(resp.shares), 1)
...
class TestSharesNotified(TestWithDatabase):
    ...
    def test_share_node_deleted(self):
        ...
        def login1(client):
            ...
            # create the share
            yield client.create_share(
                req.new_id, self.usr1.username, "name", Share.VIEW
            )
            # mark the share as accepted by hand, so we don't
            # have to emulate the whole process just for this test
        ...
        def notif2(share_id):
            ...
            # find the real share
            self.assertRaises(
                errors.DoesNotExist, self.usr1.get_share, self._state.share_id
            )
    ...
    def test_share_node_overwritten_with_move(self):
        ...
        def make_notification(_):
            ...
            d.addCallback(
                lambda _: client1.move(
                    request.ROOT,
                    self._state.srcdir.new_id,
                    self._state.parent.new_id,
                    "hi",
                )
            )
